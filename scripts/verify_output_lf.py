#!/usr/bin/env python3
"""Checkpoint 12: verify the delivered long-form MP4 directly against the
real container - duration to the frame, resolution, fps, both streams
present, audio genuinely has signal for the whole runtime.
"""
import json
import os
import subprocess
...
MP4 = os.path.join(ROOT, "out", "sonodyne-longform-298s.mp4")
...
def main():
    ...
    print(f"audio sig : peak {peak:.3f}  rms {rms:.1f} dBFS  "
          f"{n / sr:.3f}s  silent 0.5s blocks: {len(quiet)}/{n // blk}")
    if peak < 0.05:
        ok = False
    ...
    print("\n" + ("PASS - long-form output verified." if ok else "FAILED"))
